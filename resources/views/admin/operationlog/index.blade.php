                    ...
                    <div class="layui-card">
                                    ...
                                    <div class="layui-input-inline layui-show-xs-block">
                                        <select name="route_name">
                                            <option  value="">操作类型</option>
                                              @foreach($mods as $k=>$v)
                                                    <option value="{{$k}}" @isset($parm['route_name']) {{  $parm['route_name']==$k ? 'selected':''   }}    @endisset>{{$v}}</option>
                                                @endforeach
                                         </select>
                                    </div>
                        ...
                        <div class="layui-card-body ">
                            <table class="layui-table layui-form">
                                ...
                                @foreach($operationlog as $one)
                                    <tr>
                                        <td>{{$one->created_at}}</td>
                                        <td>{{$one->username}}</td>
                                        <td>{{$one->ip}}</td>
                                        <td>{{$mods[$one->route_name]?$mods[$one->route_name]:''}}</td>
                                    </tr>
                                @endforeach
                        ...
                        <div class="layui-card-body ">
                            <div class="page tcdPageCode" style="text-align: left;">
                                    <div class="row">
                                        <div class="col-xs-4">@include('admin.common.show-page-status',['result'=>$operationlog])</div>
                                        <div class="col-xs-8"><div class="dataTables_paginate paging_simple_numbers" id="dynamic-table_paginate" style="overflow: hidden;clear: both;">{!! $operationlog->links('admin.common.show-page') !!}</div></div>
                                    </div>
                            </div>
                        </div>
                    </div>
